...
def main():
	...
	input_dir=os.path.abspath(c_args['input_dir'])
	output_file=os.path.abspath(c_args['output_file'])
	priority_db=c_args['priority_db_ref']
	...
	priority_db_list=priority_db.split(",")

	priority_db_dict={v:v for i,v in enumerate(priority_db_list)}

	#### Read hits from parsed result ########

	db_hits, headers=parse_result(input_dir,priority_db_dict)
...
def parse_args(desc):
	...
	parser.add_argument("--project_path", help="Path of the project directory", required=True)
	parser.add_argument("--input_dir", help="Name and path of the input directory", required=True)
	parser.add_argument("--output_file", help="Name path to the output file", required=True)
	parser.add_argument("--priority_db_ref", help="Give comma-seperated list of user-defined blast database names in order of priority", required=True)
...
def parse_result(input_dir,priority_db_dict):
	...
	parsed_blast_file_template=os.path.join(input_dir,"*","PARSED_BLAST","*")

	list_parsed_file=glob.glob(parsed_blast_file_template)
	...
	for parsed_result_file in list_parsed_file:
